    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#F2FBFA_0%,#F7FCFB_40%,#FFFFFF_100%)] lg:flex">
      ...
      <main className="flex-1 p-4 pt-16 sm:p-6 sm:pt-20 lg:p-8 lg:pt-8">
        <Topbar
          ...
          subtitle="Consulta tu planificación diaria, revisa clientes y abre el registro de visita cuando toque iniciar o finalizar el servicio."
        ...
        <section className="mt-8 rounded-[28px] border border-[#D8EFEA] bg-white p-6 shadow-[0_16px_36px_rgba(15,25,30,0.08)] sm:p-7">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-2xl font-bold text-[#1F2937]">Agenda mensual</h2>
              <p className="mt-2 text-sm text-[#4B5563]">Servicios programados, horarios y clientes asignados a tu perfil.</p>
            </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-[#E3F2EF] bg-[#FAFDFC] p-4">
              <p className="text-sm font-semibold text-[#1F2937]">Servicios visibles</p>
              <p className="mt-2 text-3xl font-bold text-[#0F9E98]">{visibleVisits.length}</p>
            </div>
            <div className="rounded-2xl border border-[#E3F2EF] bg-[#FAFDFC] p-4">
              <p className="text-sm font-semibold text-[#1F2937]">Servicios de hoy</p>
              <p className="mt-2 text-3xl font-bold text-[#0F9E98]">{todayVisits.length}</p>
            </div>
            <div className="rounded-2xl border border-[#E3F2EF] bg-[#FAFDFC] p-4">
              <p className="text-sm font-semibold text-[#1F2937]">Próximos servicios</p>
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            <article className="rounded-[26px] border border-[#D8EFEA] bg-[#FCFFFE] p-5">
              ...
              <div className="mt-5 grid grid-cols-7 gap-2 text-center text-xs font-semibold uppercase tracking-[0.14em] text-[#6B7280]">
                ...
                {calendarDays.map((day, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex h-24 flex-col rounded-2xl border p-3 text-left transition duration-300 ${
                        isSelected
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className={`text-lg font-bold ${isSelected ? "text-[#0B4EA2]" : "text-[#1F2937]"}`}>{day.dayNumber}</span>
                        {day.visits.length > 0 && (
                          <span className="rounded-full bg-[#0F9E98] px-2 py-0.5 text-[10px] font-bold text-white">
                            {day.visits.length}
                          </span>
                        )}
                      </div>
                      ...
                      {day.visits.length > 0 ? (
                        <div className="mt-2 space-y-1 overflow-hidden text-xs text-[#4B5563]">
                        ...
                        <p className="mt-2 text-xs text-[#94A3B8]">Libre</p>
                      )}
                    </button>
                  );
                })}
            ...
            <article className="rounded-[26px] border border-[#D8EFEA] bg-white p-5">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-bold text-[#1F2937]">Servicios del día</h3>
              </div>
                    ...
                    <div key={visit.id} className="rounded-2xl border border-[#E3F2EF] bg-[#FAFDFC] p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-[#4B5563]">{formatTime(visit.fecha, i18n.language)} · {visit.estado ?? "Programado"}</p>
                        </div>
              ...
              <div className="mt-6 border-t border-[#E3F2EF] pt-5">
                <h4 className="text-sm font-bold uppercase tracking-[0.14em] text-[#6B7280]">Próximos servicios</h4>
